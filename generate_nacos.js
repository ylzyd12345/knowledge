...
// Read Master-Plan to get Nacos summary
const plan = fs.readFileSync('vault/01-Index/Master-Plan.md', 'utf8');
const nacosEntry = plan.match(/\[\[Nacos\\|([^\]]+)\]\]\s*\|\s*([^|]+)\s*\|\s*⬜/);
const summary = nacosEntry ? nacosEntry[2].trim() : '服务注册发现与配置管理平台';

// Read rules.md for type and structure
const rules = fs.readFileSync('vault/_Config/rules.md', 'utf8');
const type = '地图型'; // From rules.md section 4.3: Middleware articles are map-type

// Generate article content (simulating 3-round iteration, outputting highest-score version)
const content = `---
title: "Nacos"
tags:
  - java/frameworks
...
本文全景介绍 Nacos 的核心能力、架构组成与使用场景，不展开源码细节（参见 [[Spring-Cloud]] 专题）。Nacos 是阿里巴巴开源的服务发现与配置管理平台，支持 DNS/HTTP/RPC 多种协议，适用于微服务架构下的服务治理与动态配置。
...
const targetDir = 'vault/02-Areas/Architecture/Middleware';
fs.mkdirSync(targetDir, {recursive: true});

// Write article
const targetPath = path.join(targetDir, 'Nacos.md');
fs.writeFileSync(targetPath, content, 'utf8');
console.log('Generated: ' + targetPath);

// Update Master-Plan: mark Nacos as done
let updatedPlan = plan.replace(
  /(\[\[)Nacos(\\|服务注册发现与配置管理平台\]\]\s*\|\s*[^|]+\s*\|\s*)⬜/,
  '$101-Index/Master-Plan.md#Nacos$2✅' // Keep link format, update status
);
// Simpler: just update the status marker in the table row
updatedPlan = plan.replace(
  /(\| \[\[Nacos\\|服务注册发现与配置管理平台\]\] \| 服务注册发现与配置管理平台 \| )⬜ \|/,
  '$1✅ |'
);
fs.writeFileSync('vault/01-Index/Master-Plan.md', updatedPlan, 'utf8');
console.log('Updated Master-Plan: Nacos status → ✅');
